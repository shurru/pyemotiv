...
# dataIndex = 0
try: 
     ser= serial.Serial(6, baudrate= 57600, parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS) #need to change the serial port if another port is used.
     ...
     while 1:
         if state == 1:
            ...
            # add high and low bytes for each channel
             data = [(ord(s[i]) +ord(s[i+1])) for i in xrange(0,len(s),2) ]  
